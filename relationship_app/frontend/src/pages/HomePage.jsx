                  ...
                  <>
                    ...
                    <div className="home-note-footer">
                      ...
                      <div className="home-note-actions">
                        {note.author === currentUser?.id && <div className="home-note-privacy-control"><span>Личное</span><button type="button" className="home-small-icon" onClick={() => toggleNotePrivacy(note)} aria-label={note.is_private ? 'Сделать заметку видимой партнёру' : 'Сделать заметку личной'} aria-pressed={note.is_private} title={note.is_private ? 'Личная заметка' : 'Видно партнёру'}>{note.is_private && <Check size={14} strokeWidth={2.4} />}</button></div>}
